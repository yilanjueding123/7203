...
//			}
		...
		case LED_CHARGE_FULL:
			led_green_off();
			led_red_on();
			__msg("led_type = LED_CHARGE_FULL\r\n");
			break;
		case LED_CHARGEING:
			if (prev_mode != mode)
			{
...
void ap_peripheral_key_init(void)
{
	...
	{
		if (key_map[i].key_io == PW_KEY_PRO)
		{
			continue;
		}
		if (key_map[i].key_io)
...
void ap_peripheral_key_register(INT8U type)
{
	...
	if (type == GENERAL_KEY)
	{
		...
		key_map[1].key_active = VIDEO_ACTIVE_KEY;
